...
class UploadComponent extends Component
{
	...
	public function upload($data,$galeria) 
	{
		...
		return $this->_registry->getController()->redirect(['controller' => 'projetosprodutosentregasimgs','action' => 'add',$galeria]);
		...
		foreach ($data as $file) 
		{
			...
			$type_allowed = array('png','jpg','jpeg','gif');
					...
					elseif (is_uploaded_file($file_tmp_name)) 
					{
								...
								$file_db = TableRegistry::get('Projetosprodutosentregasimgs');
								$entity = $file_db -> newEntity();
								$entity->imagemoriginal = $filenameoriginal;
								$entity->imagempath = $dir;
								$entity->imagem = $filenameid;
								//var_dump($filename);
								$entity->projetosprodutosentrega_id = $galeria;
								$file_db->save($entity);
