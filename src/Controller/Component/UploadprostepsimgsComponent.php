 ...
 */
class UploadprostepsimgsComponent extends Component
{
    public $max_files = 3;
	...
	public function upload($data,$galeria) 
	{
		...
		return $this->_registry->getController()->redirect(['controller' => 'sistemaprostepsimgs','action' => 'add',$galeria]);
		...
		foreach ($data as $file) 
		{
			...
			$type_allowed = array('png','jpg','jpeg','gif','pptx','xls','xlsx','pdf','doc','docx','sql','prc','zip');
					...
					elseif (is_uploaded_file($file_tmp_name)) 
					{
								...
								$file_db = TableRegistry::get('Sistemaprostepsimgs');
								...
								$entity->sistemaprostep_id = $galeria;
								$file_db->save($entity);
					...
					return $this->_registry->getController()->redirect(['controller'=>'Sistemaprosteps','action' => 'view',$galeria]);	
					}
