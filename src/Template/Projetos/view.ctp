          ...
          <nav class="navbar navbar-light bg-light">
            ...
            <div class="row" >
                    ...
                    <div class="col-2"> 
                        <?= $this->Form->create($projeto, ['url'=>['controller' => 'Projetosalocs', 'action' => 'add'],'']) ?>   
                            <?= $this->Form->control('Alocacao',['default'=> $this->Number->format($projeto->id),'type' => 'hidden']); ?>
                             ...
                             <?= $this->Form->button('Atribuir Alocação',['class'=>'btn btn-info btn-sm btn-block']) ?>
                        <?= $this->Form->end() ?> 
                    </div>
                     <div class="col-2">        
                 ...
                 <div class="row" >
                    ...
                    <div class="col-2"> 
                        <?= $this->Form->create($projeto, ['url'=>['controller' => 'Projetosimages', 'action' => 'add'],'']) ?>   
                            <?= $this->Form->control('Imagem',['default'=> $this->Number->format($projeto->id),'type' => 'hidden']); ?>
                                  
                             <?= $this->Form->button('Imagem',['class'=>'btn btn-secondary btn-sm btn-block']) ?>
                        <?= $this->Form->end() ?> 
                    </div>    
                    <div class="col-2"> 
                        <?= $this->Form->create($projeto, ['url'=>['controller' => 'Projetosblockpoints', 'action' => 'addid'],'']) ?>   
                            <?= $this->Form->control('BlockPoint',['default'=> $this->Number->format($projeto->id),'type' => 'hidden']); ?>
                                  
                             <?= $this->Form->button('Impedimento',['class'=>'btn btn-secondary btn-sm btn-block']) ?>
                        <?= $this->Form->end() ?> 
                    </div> 
                    <div class="col-2"> 
                        <?= $this->Form->create($projeto, ['url'=>['controller' => 'Projetoscutovers', 'action' => 'addid'],'']) ?>   
                            <?= $this->Form->control('Cutover',['default'=> $this->Number->format($projeto->id),'type' => 'hidden']); ?>
                                  
                             <?= $this->Form->button('CutOver',['class'=>'btn btn-secondary btn-sm btn-block']) ?>
                        <?= $this->Form->end() ?> 
                    </div>
                     <div class="col-2"> 
                        <?= $this->Form->create($projeto, ['url'=>['controller' => 'Projetosurls', 'action' => 'addid'],'']) ?>   
                            <?= $this->Form->control('Url',['default'=> $this->Number->format($projeto->id),'type' => 'hidden']); ?>
                                  
                             <?= $this->Form->button('URLs/Endpoints',['class'=>'btn btn-secondary btn-sm btn-block']) ?>
                        <?= $this->Form->end() ?> 
                    </div>
                    <div class="col-2"> 
                        <?= $this->Form->create($projeto, ['url'=>['controller' => 'Projetosrestrictions', 'action' => 'addid'],'']) ?>   
                            <?= $this->Form->control('Restrict',['default'=> $this->Number->format($projeto->id),'type' => 'hidden']); ?>
                                  
                             <?= $this->Form->button('Constraints',['class'=>'btn btn-secondary btn-sm btn-block']) ?>
                        <?= $this->Form->end() ?> 
                    </div>                               
                 </div> 
...
<div class="projetos view large-9 medium-8 columns content">
    ...
    <!-- <table class="vertical-table"> -->
        ...
        <tr>
            ...
            <td><?= $projeto->ativo ? __('Yes') : __('No'); ?></td>
        </tr>
    ...
    <div class="related">
        ...
        <table class="table table-sm">
            ...
            <tr>
                ...
                <td class="actions">
                    <?= $this->Html->link(__('VIE'), ['controller' => 'Projetosprodutos', 'action' => 'view', $projetosprodutos->id],['class'=>'btn btn-primary btn-sm']) ?>
                    <?= $this->Html->link(__('EDI'), ['controller' => 'Projetosprodutos', 'action' => 'edit', $projetosprodutos->id],['class'=>'btn btn-success btn-sm']) ?>
                    <?= $this->Form->postLink(__('DEL'), ['controller' => 'Projetosprodutos', 'action' => 'delete', $projetosprodutos->id],['class'=>'btn btn-danger btn-sm'], ['confirm' => __('Are you sure you want to delete # {0}?', $projetosprodutos->id)]) ?>
                </td>
            </tr>
            <?php endforeach; ?>
        </table>
    ...
    <div class="related">
        ...
        <?php if (!empty($projeto->projetosblockpoints)): ?>
        <table class="table table-sm">
            <tr class="bg-secondary">
                <th scope="col"><?= __('Id') ?></th>            
                <th scope="col"><?= __('Descricao') ?></th>                
                <th scope="col"><?= __('Created') ?></th>
                <th scope="col"><?= __('Resolvidoem') ?></th>                                
                <th scope="col" class="actions"><?= __('Actions') ?></th>
            </tr>
            <?php foreach ($projeto->projetosblockpoints as $projetosblockpoints): ?>
            <tr>
                <td><?= h($projetosblockpoints->id) ?></td>                
                <td><?= h($projetosblockpoints->descricao) ?></td>                
                <td><?= h($projetosblockpoints->created) ?></td>
                <td><?= h($projetosblockpoints->resolvidoem) ?></td>                
                <td class="actions">
                    <?= $this->Html->link(__('View'), ['controller' => 'Projetosblockpoints', 'action' => 'view', $projetosblockpoints->id]) ?>
                    <?= $this->Html->link(__('Edit'), ['controller' => 'Projetosblockpoints', 'action' => 'edit', $projetosblockpoints->id]) ?>
                    <?= $this->Form->postLink(__('Delete'), ['controller' => 'Projetosblockpoints', 'action' => 'delete', $projetosblockpoints->id], ['confirm' => __('Are you sure you want to delete # {0}?', $projetosblockpoints->id)]) ?>
                </td>
            </tr>
            <?php endforeach; ?>
        </table>
    ...
    <div class="related">
        <h4><p class="text-secondary"><?= __('URLs/EndPoints') ?></h4>
        <?php if (!empty($projeto->projetosurls)): ?>
        <table class="table">
            <tr>
                <th scope="col"><?= __('Id') ?></th>
                <th scope="col"><?= __('Descricao') ?></th>
            ...
            <?php foreach ($projeto->projetosurls as $projetosurls): ?>
            <tr>
                <td><?= h($projetosurls->id) ?></td>
                <td><?= h($projetosurls->descricao) ?></td>
                <td><?= h($projetosurls->url) ?></td>
                <td><?= h($projetosurls->ativo) ?></td>
                <td><?= h($projetosurls->created) ?></td>
                <td class="actions">
                    <?= $this->Html->link(__('View'), ['controller' => 'Projetosalocs', 'action' => 'view', $projetosurls->id]) ?>
                    <?= $this->Html->link(__('Edit'), ['controller' => 'Projetosalocs', 'action' => 'edit', $projetosurls->id]) ?>
                    <?= $this->Form->postLink(__('Delete'), ['controller' => 'Projetosalocs', 'action' => 'delete', $projetosurls->id], ['confirm' => __('Are you sure you want to delete # {0}?', $projetosurls->id)]) ?>
                </td>
            </tr>
            <?php endforeach; ?>
        </table>
    ...
    <div class="related">
        <h4><p class="text-secondary"><?= __('CutOver') ?></h4>
        <?php if (!empty($projeto->projetoscutovers)): ?>
        <table class="table">
            <tr>
                <th scope="col"><?= __('Id') ?></th>
            ...
            <?php foreach ($projeto->projetoscutovers as $cutovers): ?>
            <tr>
                <td><?= h($cutovers->id) ?></td>
                <td><?= h($cutovers->dataemissao) ?></td>
                <td><?= h($cutovers->descricao) ?></td>
                <td><?= h($cutovers->created) ?></td>                                
                <td class="actions">
                    <?= $this->Html->link(__('View'), ['controller' => 'Projetoscutovers', 'action' => 'view', $cutovers->id]) ?>
                    <?= $this->Html->link(__('Edit'), ['controller' => 'Projetoscutovers', 'action' => 'edit', $cutovers->id]) ?>
                    <?= $this->Form->postLink(__('Delete'), ['controller' => 'Projetoscutovers', 'action' => 'delete', $cutovers->id], ['confirm' => __('Are you sure you want to delete # {0}?', $cutovers->id)]) ?>
                </td>
            </tr>
            <?php endforeach; ?>
        </table>
     ...
     <div class="related">
        <h4><p class="text-secondary"><?= __('Constraints') ?></h4>
        <?php if (!empty($projeto->projetosrestrictions)): ?>
        <table class="table">
            <tr>
                <th scope="col"><?= __('Id') ?></th>                
                <th scope="col"><?= __('Descricao') ?></th>                
            ...
            <?php foreach ($projeto->projetosrestrictions as $contraints): ?>
            <tr>
                <td><?= h($contraints->id) ?></td>                
                <td><?= h($contraints->descricao) ?></td>
                <td><?= h($contraints->created) ?></td>                                
                <td class="actions">
                    <?= $this->Html->link(__('View'), ['controller' => 'Projetosrestrictions', 'action' => 'view', $contraints->id]) ?>
                    <?= $this->Html->link(__('Edit'), ['controller' => 'Projetosrestrictions', 'action' => 'edit', $contraints->id]) ?>
                    <?= $this->Form->postLink(__('Delete'), ['controller' => 'Projetosrestrictions', 'action' => 'delete', $contraints->id], ['confirm' => __('Are you sure you want to delete # {0}?', $contraints->id)]) ?>
                </td>
            </tr>
            <?php endforeach; ?>
        </table>
